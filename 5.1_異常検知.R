...
# 時系列データの分割 (initial_time_splitを使用)
df_split <- initial_time_split(df, prop = 1 - target_length / nrow(df))

# 訓練データと評価データに分割
df_train <- training(df_split)
df_target <- testing(df_split)

# 結果の確認
print(dim(df_train))  # 訓練データのサイズ
print(dim(df_target)) # 評価データのサイズ
